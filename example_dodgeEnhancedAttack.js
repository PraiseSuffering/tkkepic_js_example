...
function getSkillId(){return "example_dodgeEnhancedAttack"}
...
function init(customJsSkill){
	...
	Module_default.addEvent("onExecuteSkillHighPriority",function(container,event){
		...
		var comboCounter=patch.getSkill(SkillSlots.BASIC_ATTACK.universalOrdinal()).getDataManager().getDataValue(SkillDataKeys.COMBO_COUNTER.get());
		var sprinting=player.m_20142_()//疾跑
		switch(event.event.skillSlot){
			...
			case SkillSlots.BASIC_ATTACK.universalOrdinal():
				//普攻
				if(!container.isEnable){break}//没激活不能用
				...
				this.data["animation"].play(player)
				container.isEnable=false;
				container.needUpdate=true;//取消启用黄边 并且更新
				dataManager.setData(SkillDataKeys.COMBO_COUNTER.get(),Integer.valueOf(0));//清除普攻段数
				break
			case SkillSlots.DODGE.universalOrdinal():
				//闪避触发
				if(container.cooldown!=0){break}
				if(container.mana!=0){break}
				container.cooldown=container.maxCooldow
				container.mana=container.maxManaaxMana
	...
	Module_default.addEvent("tick",function(container){
		if(container.maxCooldow!=0 && container.isEnable && container.cooldown==0){
			container.isEnable=false
			container.needUpdate=true;
		}
	});
...
function regSkillBook(customJsSkill){//技能书 技能工作台配置 random_skill_item skill_workbench
	...
	var displayName='{"italic":false,"extra":[{"color":"#FF4AFF","text":"example_dodgeEnhancedAttack"}],"text":""}'
	var lores=[]
	lores[0]='{"italic":false,"extra":[{"color":"#ffffff","text":"可以镶嵌在任何武器、盔甲的任何槽."}],"text":""}'
